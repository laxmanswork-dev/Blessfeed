...
export default function Feed() {
  ...
  const getMoodBorder = (mood) => {
    if (mood === "😄" || mood === "🙂" || mood === "✨") return "border-green-400";
    if (mood === "😐") return "border-slate-300";
    if (mood === "😟" || mood === "😡") return "border-red-400";
  ...
  return (
    <div className="max-w-[390px] mx-auto min-h-screen bg-white border-x border-slate-200 px-4 py-6">
      ...
      {posts.map((post) => (
        <div
          key={post._id}
          className={`border-2 rounded-xl p-4 mb-4 ${getMoodBorder(post.mood)}`}
        >
